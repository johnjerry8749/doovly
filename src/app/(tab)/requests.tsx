    ...
    return (
      <View style={styles.card}>
        <View style={styles.posterRow}>
          <Image source={{ uri: item.posterAvatar }} style={styles.posterAvatar} />
        ...
        <View style={styles.engagementRow}>
          <TouchableOpacity style={styles.engagementBtn} onPress={() => toggleLike(item.id)}>
          ...
          <TouchableOpacity style={styles.engagementBtn} onPress={() => openChat(item)}>
            <Ionicons name="chatbubble-outline" size={18} color="#6B7280" />
            <Text style={styles.engagementText}>{commentCount}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.engagementBtn} onPress={() => shareRequest(item)}>
            <Ionicons name="share-outline" size={18} color="#6B7280" />
            <Text style={styles.engagementText}>Share</Text>
          </TouchableOpacity>
        </View>
        {firstComment ? (
          <TouchableOpacity style={styles.commentPreview} onPress={() => openChat(item)}>
            <Image source={{ uri: firstComment.userAvatar }} style={styles.commentAvatar} />
            <View style={styles.commentBody}>
              <Text style={styles.commentName}>{firstComment.userName}</Text>
              <Text style={styles.commentText} numberOfLines={2}>{firstComment.text}</Text>
            </View>
          </TouchableOpacity>
        ) : (
          <TouchableOpacity style={styles.writeCommentHint} onPress={() => openChat(item)}>
        ...
        <TouchableOpacity style={styles.sendOfferBtn} onPress={() => setOfferRequest(item)}>
    ...
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      <View style={styles.header}>
        <View style={styles.headerTextWrap}>
          ...
          <TouchableOpacity style={styles.headerLocationRow} onPress={() => setShowLocationModal(true)} activeOpacity={0.7}>
            <Ionicons name="location" size={14} color={GREEN} />
            {loadingLocation ? (
              <ActivityIndicator size="small" color={GREEN} style={{ marginLeft: 4 }} />
            ) : (
              <Text style={styles.headerSubtitle} numberOfLines={1}>{locationName || "All Nigeria"}</Text>
            )}
            <Ionicons name="chevron-down" size={14} color="#6B7280" />
          </TouchableOpacity>
        </View>
        <TouchableOpacity style={styles.createBtn} onPress={() => router.push("/profile/createjob")}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TextInput style={styles.searchInput} placeholder="Search service requests..." placeholderTextColor="#888" value={search} onChangeText={setSearch} />
      </View>
      ...
      <View style={styles.filtersWrap}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filtersContent}>
          {CATEGORY_FILTERS.map((cat) => {
            ...
            return (
              <TouchableOpacity key={cat} style={[styles.filterChip, active && styles.filterChipActive]} onPress={() => setCategoryFilter(cat)}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>No service requests in this area yet.</Text>
          </View>
        }
      ...
      <Modal visible={!!chatRequest} transparent animationType="slide" onRequestClose={closeChat}>
        <KeyboardAvoidingView style={styles.cmBackdrop} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          ...
          <View style={styles.cmSheet}>
            ...
            <FlatList
              ...
              style={styles.cmList}
              contentContainerStyle={chatComments.length === 0 ? styles.cmListEmptyContent : styles.cmListContent}
              keyboardShouldPersistTaps="handled"
              ListEmptyComponent={
                <View style={styles.cmEmpty}>
                  <Ionicons name="chatbubbles-outline" size={40} color="#D1D5DB" />
                  <Text style={styles.cmEmptyTitle}>No comments yet</Text>
                  <Text style={styles.cmEmptyText}>Be the first to share your thoughts.</Text>
                </View>
              }
            />
            {replyTo ? (
            ...
            <View style={styles.cmInputRow}>
              ...
              <TextInput
                ...
                placeholder={replyTo ? `Reply to ${replyTo.userName}…` : "What do you think of this?"}
              ...
              />
              {chatText.trim() ? (
                <TouchableOpacity style={styles.cmPostBtn} onPress={sendChatMessage}>
      ...
      <Modal visible={!!offerRequest} transparent animationType="slide" onRequestClose={closeOffer}>
        <KeyboardAvoidingView style={styles.offerBackdrop} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <Pressable style={styles.offerBackdrop} onPress={closeOffer}>
            <Pressable style={styles.offerSheet} onPress={(e) => e.stopPropagation()}>
              ...
              {offerRequest ? <Text style={styles.offerSubtitle} numberOfLines={2}>{offerRequest.title}</Text> : null}
              <Text style={styles.offerLabel}>Your price</Text>
              <View style={styles.offerField}>
                <Text style={styles.offerNaira}>₦</Text>
                <TextInput value={offerPrice} onChangeText={setOfferPrice} placeholder="Enter amount" placeholderTextColor="#9CA3AF" keyboardType="numeric" style={styles.offerInput} />
              </View>
              <TouchableOpacity
                style={[styles.offerSubmitBtn, !offerPrice.replace(/[^\d]/g, "") && styles.offerSubmitDisabled]}
      ...
      <Modal visible={showLocationModal} transparent animationType="slide" onRequestClose={() => setShowLocationModal(false)}>
        <Pressable style={styles.locOverlay} onPress={() => setShowLocationModal(false)}>
          <View style={styles.locSheet}>
            ...
            <TouchableOpacity style={styles.locOption} onPress={getUserLocation}>
              ...
              <View style={styles.locOptionText}>
                ...
                <Text style={styles.locOptionSub}>Allow access to detect your position</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={styles.locOption} onPress={() => { setShowLocationModal(false); setShowCityPicker(true); }}>
              <Ionicons name="list-outline" size={24} color={GREEN} />
              <View style={styles.locOptionText}>
                <Text style={styles.locOptionTitle}>Select a city</Text>
                <Text style={styles.locOptionSub}>Pick from popular cities in Nigeria</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={styles.locOption} onPress={viewAllInNigeria}>
              ...
              <View style={styles.locOptionText}>
                ...
                <Text style={styles.locOptionSub}>See requests from every city</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={styles.locCancel} onPress={() => setShowLocationModal(false)}>
        ...
        <View style={styles.locOverlay}>
          <View style={[styles.locSheet, styles.locCitySheet]}>
            ...
            <View style={styles.locSearchBox}>
              ...
              <TextInput style={styles.locSearchInput} placeholder="Filter cities..." placeholderTextColor="#888" value={citySearch} onChangeText={setCitySearch} autoCorrect={false} />
              {citySearch.length > 0 ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={<Text style={styles.locEmptyCities}>No city found.</Text>}
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.locCityItem} onPress={() => selectCity(item)}>
...
const styles = StyleSheet.create({
  ...
  createBtn: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: GREEN, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20 },
  createBtnText: { color: "#fff", fontSize: 13, fontWeight: "700" },
  searchContainer: { height: 46, borderWidth: 1, borderColor: "#E5E5E5", borderRadius: 23, flexDirection: "row", alignItems: "center", paddingHorizontal: 14, marginHorizontal: 16, marginBottom: 10 },
  searchInput: { flex: 1, fontSize: 15, color: "#111", marginHorizontal: 8, paddingVertical: 0 },
  filtersWrap: { height: 44, marginBottom: 8 },
  filtersContent: { paddingHorizontal: 16, alignItems: "center", gap: 8 },
  filterChip: { height: 34, paddingHorizontal: 14, borderRadius: 17, backgroundColor: "#F3F4F6", justifyContent: "center", alignItems: "center", marginRight: 4 },
  filterChipActive: { backgroundColor: GREEN },
  filterChipText: { fontSize: 13, fontWeight: "600", color: "#6B7280" },
  filterChipTextActive: { color: "#fff" },
  listContent: { paddingHorizontal: 16, paddingBottom: 20, gap: 14 },
  card: { borderWidth: 1, borderColor: "#E8E8E8", borderRadius: 16, padding: 14, backgroundColor: "#fff" },
  ...
  cardTitle: { flex: 1, fontSize: 16, fontWeight: "800", color: "#111", lineHeight: 22 },
  newBadge: { backgroundColor: "#F59E0B", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  newBadgeText: { color: "#fff", fontSize: 10, fontWeight: "800" },
  coverImage: { width: "100%", height: 180, borderRadius: 12, backgroundColor: "#F3F4F6", marginBottom: 10 },
  ...
  engagementRow: { flexDirection: "row", alignItems: "center", gap: 18, marginBottom: 12, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: "#F3F4F6" },
  engagementBtn: { flexDirection: "row", alignItems: "center", gap: 5 },
  engagementText: { fontSize: 13, color: "#6B7280", fontWeight: "500" },
  commentPreview: { flexDirection: "row", alignItems: "flex-start", gap: 8, marginBottom: 6 },
  commentAvatar: { width: 28, height: 28, borderRadius: 14, backgroundColor: "#E5E7EB" },
  commentBody: { flex: 1, backgroundColor: "#F9FAFB", borderRadius: 12, paddingHorizontal: 10, paddingVertical: 8 },
  commentName: { fontSize: 12, fontWeight: "700", color: "#111", marginBottom: 2 },
  commentText: { fontSize: 12, color: "#4B5563", lineHeight: 17 },
  viewMoreComments: { fontSize: 12, color: GREEN, fontWeight: "600", marginBottom: 12, marginLeft: 36 },
  writeCommentHint: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 12 },
  writeCommentHintText: { fontSize: 13, color: GREEN, fontWeight: "600" },
  sendOfferBtn: { backgroundColor: GREEN, borderRadius: 24, paddingVertical: 12, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  ...
  cmDim: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.45)" },
  cmSheet: { backgroundColor: "#fff", borderTopLeftRadius: 16, borderTopRightRadius: 16, maxHeight: "78%", minHeight: "55%", paddingBottom: Platform.OS === "ios" ? 28 : 12 },
  cmHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: "#D1D5DB", alignSelf: "center", marginTop: 10, marginBottom: 6 },
  cmHeader: { alignItems: "center", justifyContent: "center", paddingVertical: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: "#E5E7EB" },
  cmHeaderTitle: { fontSize: 16, fontWeight: "700", color: "#111" },
  cmCloseBtn: { position: "absolute", right: 14, top: 8, width: 32, height: 32, alignItems: "center", justifyContent: "center" },
  cmList: { flexGrow: 1 },
  cmListContent: { paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8 },
  cmListEmptyContent: { flexGrow: 1, justifyContent: "center", paddingVertical: 40 },
  cmEmpty: { alignItems: "center", paddingVertical: 40 },
  cmEmptyTitle: { fontSize: 16, fontWeight: "700", color: "#111", marginTop: 12 },
  cmEmptyText: { fontSize: 13, color: "#9CA3AF", marginTop: 4 },
  cmRow: { flexDirection: "row", alignItems: "flex-start", marginBottom: 18 },
  cmAvatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#E5E7EB", marginRight: 12 },
  ...
  cmReply: { fontSize: 12, fontWeight: "600", color: "#9CA3AF", marginTop: 6 },
  cmReplyBar: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 8, backgroundColor: "#F9FAFB", borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: "#E5E7EB" },
  cmReplyBarText: { fontSize: 13, color: "#6B7280", flex: 1 },
  cmReplyBarName: { fontWeight: "700", color: "#111" },
  cmInputRow: { flexDirection: "row", alignItems: "flex-end", paddingHorizontal: 14, paddingTop: 10, paddingBottom: 6, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: "#E5E7EB", gap: 10 },
  cmInputAvatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: "#E5E7EB", marginBottom: 4 },
  cmInput: { flex: 1, minHeight: 40, maxHeight: 100, fontSize: 15, color: "#111", paddingVertical: 10 },
  cmPostBtn: { paddingHorizontal: 8, paddingVertical: 8 },
  cmPostText: { fontSize: 15, fontWeight: "700", color: GREEN },
  offerBackdrop: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.4)" },
  offerSheet: { backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 12, paddingBottom: 32 },
  offerHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: "#D1D5DB", alignSelf: "center", marginBottom: 14 },
  offerTitle: { fontSize: 18, fontWeight: "800", color: "#111827", textAlign: "center" },
  offerSubtitle: { fontSize: 13, color: "#6B7280", textAlign: "center", marginTop: 4, marginBottom: 16 },
  offerLabel: { fontSize: 13, fontWeight: "600", color: "#111827", marginBottom: 8 },
  offerField: { minHeight: 52, borderWidth: 1, borderColor: "#E5E7EB", borderRadius: 14, paddingHorizontal: 14, marginBottom: 18, flexDirection: "row", alignItems: "center", gap: 8 },
  offerNaira: { fontSize: 16, color: "#9CA3AF", fontWeight: "600" },
  offerInput: { flex: 1, fontSize: 16, color: "#111827", paddingVertical: 12 },
  offerSubmitBtn: { backgroundColor: GREEN, borderRadius: 26, paddingVertical: 14, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  offerSubmitDisabled: { opacity: 0.45 },
  offerSubmitText: { color: "#fff", fontSize: 16, fontWeight: "700" },
  locOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "flex-end" },
  locSheet: { backgroundColor: "#fff", borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingHorizontal: 20, paddingBottom: 30, paddingTop: 12 },
  locCitySheet: { maxHeight: "80%" },
  locHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: "#ddd", alignSelf: "center", marginBottom: 16 },
  locTitle: { fontSize: 18, fontWeight: "800", color: "#111", marginBottom: 18 },
  locOption: { flexDirection: "row", alignItems: "center", paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: "#F0F0F0" },
  ...
  locSearchBox: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: "#E5E5E5", borderRadius: 12, paddingHorizontal: 12, height: 44, marginBottom: 12 },
  locSearchInput: { flex: 1, fontSize: 15, color: "#111", marginLeft: 8 },
  locCityList: { maxHeight: 320 },
  locCityItem: { flexDirection: "row", alignItems: "center", paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: "#F5F5F5" },
  locCityItemText: { flex: 1, fontSize: 15, color: "#111", marginLeft: 12 },
  locEmptyCities: { textAlign: "center", color: "#888", marginTop: 30, fontSize: 14 },
});
